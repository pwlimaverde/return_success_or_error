...
Ambos são `abstract base class` (Dart 3). O `call(parameters)` posicional vem de um
`base mixin` compartilhado (`_UsecaseRunner`). Se o construtor receber `runInIsolate: true`,
o `call` executa o `run` em `Isolate.run` (convertendo falhas do isolate em `ErrorReturn`
com `Cod. IsolateCatch`); caso contrário, roda direto. A medição de tempo é **opcional** e
desligada por padrão: só quando o construtor recebe `monitorExecutionTime: true` o `call`
mede com `Stopwatch` e loga via `dart:developer`. Mantê-la `false` em produção evita o custo
de medição/log.
